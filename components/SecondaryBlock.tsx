...
const SecondaryBlock = ({ imageLink, text, link }: IProps) => {
    return(
        <Link href={`${link}`} className="news-main-news block h-[480px] w-[1320px] relative rounded-[16px] overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 w-[644px] h-full flex items-end py-[30px] px-[40px] bg-IMAGE z-10 rounded-[16px]">
                <h3 className="news-secondary-title text-WHITE font-bold text-SMALL_TITLE max-w-[605px]">{text}</h3>
            </div>
        </Link>
    )
}
